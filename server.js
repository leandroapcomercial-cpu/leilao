/**
 * LEILÃO FÁCIL v2.0 - Servidor Principal
 * Node.js + Express + Socket.IO + SQLite + PIX Mercado Pago
 */
...
async function iniciarTimerCampanha(campanhaId) {
  ...
  console.log(`⏱️ Timer iniciado para campanha ${campanhaId}: ${Math.round(msRestante / 1000)}s restantes`);
...
function cancelarTimerCampanha(campanhaId) {
  if (timersAtivos.has(campanhaId)) {
    ...
    console.log(`⏱️ Timer cancelado para campanha ${campanhaId}`);
  }
}
  ...
  socket.on('novo_lance', async (data) => {
    ...
    try {
      ...
      if (valor < item.lance_minimo) {
        socket.emit('erro_lance', { mensagem: `Lance mínimo: ${formatarMoeda(item.lance_minimo)}` });
        return;
      }
      ...
      io.to(`campanha_${campanhaId}`).emit('lance_recebido', {
        ...
        itemAtualizado: {
          ...
          vencedorNome: usuario?.nome || 'Anônimo'
        }
      });
...
app.post('/api/usuarios', async (req, res) => {
  try {
    ...
    if (!nome || nome.trim().length < 3) {
      return res.status(400).json({ erro: 'Nome é obrigatório (mínimo 3 caracteres)' });
    }
    ...
    if (type === 'payment' && data && data.id) {
      ...
      if (pagamento) {
        ...
        if (status === 'approved') {
          ...
          db.atualizarStatusPagamento(pagamento.id, 'rejeitado', status);
        }
      }
    }
...
app.post('/api/admin/login', loginLimiter, async (req, res) => {
  try {
    ...
    if (ADMIN_PASSWORD_HASH) {
      ...
      valido = senha === process.env.ADMIN_PASSWORD_PLAIN;
    }
...
app.post('/api/admin/campanhas', verificarTokenAdmin, upload.single('premio_imagem'), async (req, res) => {
  try {
    ...
    if (!dados.nome) {
      return res.status(400).json({ erro: 'Nome da campanha é obrigatório' });
    }
